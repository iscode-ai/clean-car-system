...
function Usuarios({ user }: { user: User }) {
  ...
  const [form, setForm] = useState({ nome: "", email: "", senha: "", telefone: "", role: "operador" });
  const [salvando, setSalvando] = useState(false);
  const [erro, setErro] = useState("");
  ...
  async function salvar(e: React.FormEvent) {
    ...
    const data = await res.json();
    if (res.ok) {
      setForm({ nome: "", email: "", senha: "", telefone: "", role: "operador" });
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={salvar} className="card p-5 space-y-4">
        <p className="text-sm font-medium">Criar operador ou admin</p>
        <div className="grid grid-cols-2 gap-4">
          <label className="col-span-2 block">
            <span className="label text-xs">Nome</span>
            <input required className="input mt-1.5 text-sm" value={form.nome}
          ...
          <label className="block">
            <span className="label text-xs">E-mail</span>
            <input required type="email" className="input mt-1.5 text-sm" value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value })} />
          </label>
          <label className="block">
            <span className="label text-xs">Senha inicial</span>
            <input required type="password" minLength={6} className="input mt-1.5 text-sm" value={form.senha}
              onChange={(e) => setForm({ ...form, senha: e.target.value })} />
          </label>
          <label className="block">
            <span className="label text-xs">WhatsApp (opcional)</span>
            <input className="input mt-1.5 text-sm" value={form.telefone}
              onChange={(e) => setForm({ ...form, telefone: e.target.value })} />
          </label>
          <label className="block">
            <span className="label text-xs">Função</span>
        ...
        {erro && <p className="text-sm" style={{ color: "var(--color-danger)" }}>{erro}</p>}
        {sucesso && <p className="text-sm" style={{ color: "var(--color-accent)" }}>{sucesso}</p>}
        <button disabled={salvando} className="btn-primary text-sm px-4 py-2.5">
        ...
        {usuarios.map((u) => (
          <div key={u.uid} className="card p-4 flex items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--color-text-muted)" }}>{u.email || u.uid}</p>
            </div>
